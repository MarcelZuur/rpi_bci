...
class Classifier(object):
    ...
    def plot_model(self, data):
        ...
        coefs_matrix = self.optimal_model.coef_.reshape(3, self.optimal_model.coef_.shape[1] / len(
            classifier.channel_idxs), len(classifier.channel_idxs))
        for i in range(3):
...
class SVMClassifier(Classifier):
    ...
    def __init__(self, fsample=256.0, channel_idxs=np.arange(9, 22)):
        ...
        c_grid = [0.001, 0.01, 1, 10]
        self.clf = GridSearchCV(sklearn.svm.LinearSVC(penalty="l1", dual=False), param_grid={'C': c_grid}, cv=3,
                                scoring="accuracy")
